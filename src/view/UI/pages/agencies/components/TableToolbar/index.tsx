...
const TableToolbar: FC<ITableToolbarProps> = ({numSelected}) => {
  ...
  return (
    <Toolbar sx={toolbarStyle}>
     ...
     <ToolbarActionIcons numSelected={numSelected} deleteAgency={deleteAgency} addAgency={addAgency} editAgency={editAgency}/>
    </Toolbar>
  );
};
  ...
  return (
      <Box sx={{ flex: '1 1 100%' }}> 
        <Box
          ...
          noValidate
          autoComplete="off">
          <TextField id="standard-basic" label="Поиск по названию" variant="standard"/>
        </Box>
      </Box>
  )
}
